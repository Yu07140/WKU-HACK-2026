    ...
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <Card className="space-y-4 p-5">
        ...
        <Button className="w-full" onClick={() => generate()} disabled={loading}>
          ...
          {loading ? t("Writing script...", "正在生成脚本…") : t("Generate 15s Script", "生成 15 秒脚本")}
        </Button>
        <p className="text-[11px] leading-relaxed text-ink/40">
          {t(
            "Offline template engine — works without any API key. Replace",
            "离线模板引擎 — 无需任何 API Key。将 "
          )}
      ...
      <Card className="p-6">
        {!script && !loading && (
          <div className="flex h-full min-h-64 items-center justify-center text-center text-ink/40">
            <div>
              ...
              <p className="text-sm">
                {t(
                  "Generate a structured 15-second TikTok / Reels script: 4 scenes with visuals, on-screen text, voiceover and CTA.",
                  "生成结构化的 15 秒 TikTok / Reels 脚本：4 个分镜，含画面、屏幕字幕、口播和 CTA。"
                )}
              </p>
            </div>
          </div>
        ...
        {loading && (
          <div className="flex h-full min-h-64 items-center justify-center text-sm font-semibold text-ink/50">
            <Loader2 size={18} className="mr-2 animate-spin" /> {t("Writing script...", "正在生成脚本…")}
          </div>
        )}
        {script && (
          <div className="animate-fade-up">
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-accent px-3 py-1 text-xs font-bold text-white">
                {t("15s · TikTok / Reels", "15 秒 · TikTok / Reels")}
              </span>
            ...
            <div className="space-y-3">
              {script.scenes.map((s) => (
                <div key={s.time} className="rounded-xl border border-ink/10 p-4">
                  ...
                  <div className="mt-2 space-y-1 text-sm leading-relaxed text-ink/75">
                    <p>
                      <span className="font-bold text-ink/55">{t("Visual:", "画面：")}</span> {s.visual}
                    </p>
                    <p>
                      <span className="font-bold text-ink/55">{t("On-screen text:", "屏幕字幕：")}</span>{" "}
                      {s.text}
                    </p>
                    <p>
                      <span className="font-bold text-ink/55">{t("Voiceover:", "口播：")}</span>{" "}
                      {s.voiceover}
                    </p>
                  </div>
                </div>
              ...
              {script.scenes[3] && (
                <div className="rounded-xl bg-cream p-3 text-xs text-ink/60">
                  <span className="font-bold text-ink/70">{t("End CTA:", "结尾 CTA：")}</span>{" "}
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              >
                ...
                {copied ? t("Script copied!", "脚本已复制！") : t("Copy Script", "复制脚本")}
              </button>
              <button
                onClick={() => {
                  const v = variant + 1;
              ...
              >
                <RotateCcw size={14} /> {t("Regenerate", "重新生成")}
              </button>
            </div>
          </div>
        )}
